...
//* TWINKLES IN DAERK MODE!!
function addStars() {
  if (document.querySelector('.stars')) return; // Prevent multiple star layers

  const starContainer = document.createElement("div");
  starContainer.classList.add("stars");

  for (let i = 0; i < 150; i++) { // Adjust number of stars here
    let star = document.createElement("div");
    star.classList.add("star");

    star.style.top = `${Math.random() * 100}vh`;
    star.style.left = `${Math.random() * 100}vw`;
    star.style.animationDuration = `${Math.random() * 3 + 1}s`; // Random twinkle speed
...
// Run on page load if dark mode is active
if (document.body.classList.contains("dark-mode")) {
  addStars();
}
